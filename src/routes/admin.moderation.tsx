...
import { CircleAlert as AlertCircle, Trash2, CircleCheck as CheckCircle } from "lucide-react";
...
function Moderation() {
  ...
  const approveContent = (contentId: string) => {
    setReportedContent(
      reportedContent.map((c) => (c.id === contentId ? { ...c, status: "cleared" } : c)),
    );
  };
            ...
            {reportedContent.map((item) => (
              <Card key={item.id} className="p-6">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <Badge className={getReasonBadgeClass(item.reason)}>{item.reason}</Badge>
                </div>
                  ...
                  <tr key={user.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm">
                      {user.linkedAccounts.length > 0 ? (
                        <span className="text-gray-600">{user.linkedAccounts.join(", ")}</span>
                      ) : (
