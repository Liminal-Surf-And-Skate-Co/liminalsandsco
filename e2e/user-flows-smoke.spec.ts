...
test.describe("User Interactive Flows - Smoke Tests", () => {
  ...
  test("should browse products without errors", async ({ page }) => {
    ...
    const productsLink = page.locator(
      'a[href*="product"], a:has-text("Products"), a:has-text("Shop")',
    );
    ...
    const fatalErrors = errors.filter(
      (e) => !e.includes("favicon") && !e.includes("404") && !e.includes("Network request failed"),
    );
  ...
  test("should handle newsletter signup without state loss", async ({ page }) => {
    ...
    if ((await emailInput.count()) > 0) {
      ...
      const submitButton = page.locator(
        'button[type="submit"], button:has-text("Subscribe"), button:has-text("Sign Up")',
      );
  ...
  test("should handle review submission flow", async ({ page, context }) => {
    ...
    const reviewForm = page.locator(
      'form[aria-label*="review"], form:has-text("Review"), .review-form',
    );
